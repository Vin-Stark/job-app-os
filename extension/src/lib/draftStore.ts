...
import type { ScrapedJob, JdMeta, EligibilityCheck, Recommendation, SkillsBreakdown } from './types'

const DRAFT_KEY = 'jobappos_draft'
const DRAFT_TTL_MS = 24 * 60 * 60 * 1000

// Only settled phases are restorable — transient ones (analyzing, tailoring…)
// degrade to the nearest settled phase at save time so a reopen never lands
// on a dead spinner. 'scored' is settled: the user is mid-decision there.
export type DraftPhase = 'review' | 'scored' | 'ineligible' | 'done'

export interface Draft {
  v: 3 // schema version — bump to invalidate persisted drafts on shape changes
  savedAt: number
  ...
  // JD skills (must-have + preferred keywords) not found in the resume,
  // must-haves first — the count is what the UI shows
  missingSkills: string[] | null
  // Categorized gap view for the scored screen (must-have / proof-based /
  // trainable / not covered)
  skills: SkillsBreakdown | null
  jobId: number | null
...
export async function getDraft(): Promise<Draft | null> {
  ...
  if (!draft || draft.v !== 3 || Date.now() - draft.savedAt > DRAFT_TTL_MS) {
...
export async function setDraft(d: Omit<Draft, 'v' | 'savedAt'>): Promise<void> {
  const draft: Draft = { ...d, v: 3, savedAt: Date.now() }
